...
export class MerkleDatabase {
  ...
  private entriesToPollardMap: Map<string, CID>;
  ...
  constructor({
    ...
    this.layers = [];
    this.orderedEntriesMap = new OrderedMap(
      [],
    ...
    this.entriesToPollardMap = new Map();
  ...
  async put(key: string, value: object): Promise<void> {
    const { cid, entry } = await createEntry(
      key,
      value,
      this.identity,
      this.ipfs
  ...
  async updateLayers(sortKey: number): Promise<void> {
    ...
    const [startKey, startCid] =
      this.orderedEntriesMap.getElementByPos(indexStart);
    ...
    while (this.layers[layerIndex - 1].length > 1) {
      ...
      position = Math.floor(
        startPosition / this.maxPollardLength ** layerIndex
      );
      const startIndexInLowerLayer = position * this.maxPollardLength;
      const slicedLayer = this.layers[layerIndex - 1].slice(
        startIndexInLowerLayer
      );
  ...
  getPollardTreeNode({ layerIndex, position }: PollardNode): PollardNode {
    if (
      this.layers.length <= layerIndex ||
      this.layers[layerIndex].length <= position
    ) {
  ...
  getPollardTreeNodeLeft(node: PollardNode): PollardNode {
    ...
    if (
      node.position >=
      Math.ceil(
        this.orderedEntriesMap.size() /
          2 ** (this.pollardOrder * (node.layerIndex + 1))
      )
    )
  ...
  async load(rootPollardCid: CID): Promise<void> {
    let leaves: LeafType[] = [
      createLeaf(LeafTypes.Pollard, rootPollardCid.bytes),
    ];
  ...
  async loadData(): Promise<void> {
    for (const [key, cid] of this.orderedEntriesMap) {
      ...
      if (verified) {
        const decoded = json.decode(verified);
        console.log({ decoded });
