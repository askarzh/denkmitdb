...
async function exportPrivateKey(
  keys: KeyPair,
  passphrase: string
): Promise<jose.FlattenedJWE> {
  if (!keys.privateKey) {
    throw new Error("Private key is not available");
  }
  const encryptedPrivateKey = await new jose.FlattenedEncrypt(
    json.encode(await jose.exportJWK(keys.privateKey))
  )
...
async function importPrivateKey(
  ...
  const privateJwk = json.decode(result.plaintext) as jose.JWK;
  const publicJwk = {
    kty: privateJwk.kty,
    x: privateJwk.x,
    y: privateJwk.y,
  ...
  const publicKey = uint8ArrayToString(json.encode(publicJwk), "base64");
...
async function createJWS(
  ...
  return await new jose.FlattenedSign(payload)
...
export async function getIdentity(
  ...
  const verifyResult = await jose.flattenedVerify(
    identityJWS,
    jose.EmbeddedJWK
  );
  const identityInput: IdentityInput = json.decode(verifyResult.payload);
  const id = cid.toString(base64.encoder);
  const identity: IdentityType = { ...identityInput, id };

  const publicJwk = json.decode(
    uint8ArrayFromString(identity.publicKey, "base64")
  );
  const publicKey = (await jose.importJWK(
    publicJwk as jose.JWK
  )) as jose.KeyLike;
...
export async function createIdentity(
  ...
  if (await ipfs.datastore.has(key)) {
    ...
    const { cid, encryptedPrivateKey } = json.decode(data) as IdentityDatastore;

    const importedKeys = await importPrivateKey(
      encryptedPrivateKey,
      passphrase
    );
  ...
  const identityJWS = await createJWS(json.encode(identityToSign), keys, {
    alg,
    includeJwk: true,
  });
  ...
  await ipfs.datastore.put(key, json.encode(identityDatastore));

  return new Identity(identity, identityJWS, keys);
}

export async function createIdentityFromJWS(
  id: string,
  jws: IdentityJWS,
  keys: KeyPair
): Promise<IdentityInterface> {
  const verifyResult = await jose.flattenedVerify(jws, jose.EmbeddedJWK);
  const IdentityInput: IdentityInput = json.decode(verifyResult.payload);
  const identity: IdentityType = { ...IdentityInput, id };

  return new Identity(identity, jws, keys);
}

export async function createIdentityFromBlock(
  bytes: Uint8Array,
  keys?: KeyPair
): Promise<IdentityInterface> {
  const block = (await Block.decode({
    ...
    codec: DagCborCodec,
    hasher: sha256,
  })) as BlockView<IdentityJWS>;

  const verifyResult = await jose.flattenedVerify(
    block.value,
    jose.EmbeddedJWK
  );
  const identity: IdentityType = DagCborCodec.decode(verifyResult.payload);
  identity.id = block.cid.toString(base64.encoder);
  ...
  return new Identity(identity, block.value, { ...keys, publicKey });
}
